...
void imprimirTabuleiro() {
    ...
    for (int i = 1; i < TAMANHO_TABULEIRO + 1; i++) {
        cout << setw(LARGURA_COLUNA) << i; // Imprime o número da coluna com largura fixa
    }
    cout << endl;
    ...
    for (int i = 0; i < TAMANHO_TABULEIRO; i++) {
        cout << setw(2) << vetorLetras[i] << " "; // Imprime a letra da linha com espaçamento fixo

        for (int j = 0; j < TAMANHO_TABULEIRO; j++) {
            cout << setw(LARGURA_COLUNA) << tabuleiro[i][j]; // Imprime o conteúdo do tabuleiro com largura fixa
        }
        cout << endl;
    }
}
