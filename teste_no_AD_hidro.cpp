...
{
public:
	Barcos(int tamanho, int quantidade) : tamanho(tamanho), quantidade(quantidade) {}
	int tamanho = 0;
	int quantidade = 0;
...
void imprimirTabuleiro(char tabuleiro[TAMANHO_TABULEIRO][TAMANHO_TABULEIRO])
{
	...
	for (int i = 0; i < TAMANHO_TABULEIRO; i++)
	{
		...
		for (int j = 0; j < TAMANHO_TABULEIRO; j++)
		{
			...
			else if (tabuleiro[i][j] == HIDROAVIAO) // black  word
			{
			...
			if (linhaLetra1 == 0 || linhaLetra1 == 14)
			{
				...
				if (linhaLetra1 == 0)
				{
					...
					if (colunaNumero2 == colunaNumero1)
					{
						...
						//Nessa condição a linha letra sempŕe vai ser 2
						linhaLetra2 = 2;

						colunaNumero3 = hidrohaviao1.LerIndiceNumero(entradaValida, j + 2, H);
						linhaLetra3 = 1;
						// Neste if a 3 coluna3 tem que atender uma das condições abaixo
